...
&lt;/script&gt;</div>
  ...
  const clientScript = `
    ...
    const loadPageSettings = function() {
      try {
    ...
    AICRO.openSelector = AICRO.openSelector || function() {
    ...
    AICRO.showBookmarkletHelper = AICRO.showBookmarkletHelper || function() {
    ...
    // Add essential initialization function that was missing
    AICRO.init = AICRO.init || function(options = {}) {
      ...
      if (config.debug) {
        ...
        const addButtonToDOM = function() {
          if (document && document.body) {
            ...
            // Helper method to get a selector URL
            AICRO.getSelectorURL = function() {
    ...
    const initAndLoadSettings = function() {
      try {
    ...
    const checkForSelectorModeParam = function() {
      try {
        ...
        if (isSelectorMode) {
          ...
          setTimeout(() => {
            // Launch the selector
            if (typeof AICRO.openSelector === 'function') {
              AICRO.openSelector();
            }
          }, 500);
        }
